...
async function runFoodSearch(query) {
  ...
  try {
    ...
    box.querySelectorAll('button[data-idx]').forEach((btn) => {
      btn.addEventListener('click', () => addFoodToMeal(results[Number(btn.dataset.idx)]));
    });
  } catch (err) {
...
async function addFoodToMeal(food) {
  const gramsStr = prompt(`Grams of "${food.description}" consumed:`, '100');
...
async function exportData() {
  const [p, meals, symptoms, checkins] = await Promise.all([
    DB.getProfile(), DB.allMeals(), DB.allSymptoms(), DB.allCheckins(),
  ]);
  const blob = new Blob([JSON.stringify({ profile: p, meals, symptoms, checkins }, null, 2)], { type: 'application/json' });
